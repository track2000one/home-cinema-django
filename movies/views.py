...
def resolve_local_video_path(
    ...
    if not raw_path:
        raise Http404(
            "No local video file is configured."
        )
...
def stream_video(request, pk):
    ...
    if not movie.video_path:
        raise Http404(
            "This movie uses Google Drive "
            "and has no local video file."
        )

    path = resolve_local_video_path(
        movie.video_path
    )
